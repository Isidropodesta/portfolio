...
/*
 * Layout (flujo normal, sin absolute):
 *
 *   ┌──────────────────────────┐
 *   │    [avatar wrapper]      │  230px visible (overflow hidden)
 *   │    overflow: hidden      │  → muestra head→waist de la img de 320px
 *   └──────────────────────────┘  ← base del avatar = borde sup. del chat
 *   ┌──────────────────────────┐
 *   │       CHAT BOX           │
 *   └──────────────────────────┘
 *
 *  Float: GSAP anima el wrapper (translateY) → el clip "flota" completo.
 *  Bounce: GSAP scale sobre el img.
 *  Sin margin, sin gap → base avatar = borde chat.
 */
...
export default function Hero() {
  ...
  // Float se aplica al wrapper (el clip flota completo sin cortar la cabeza)
  const avatarWrapperRef = useRef<HTMLDivElement>(null);
  // Bounce se aplica al img
  const avatarImgRef     = useRef<HTMLImageElement>(null);
  const floatTween       = useRef<gsap.core.Tween | null>(null);
  const bottomRef        = useRef<HTMLDivElement>(null);

  // ── Flotación continua del wrapper ────────────────────────────────────────
  const startFloat = useCallback(() => {
    floatTween.current?.kill();
    if (!avatarWrapperRef.current) return;
    floatTween.current = gsap.to(avatarWrapperRef.current, {
      y: -12,
      duration: 3,
      ease: 'sine.inOut',
      yoyo: true,
  ...
  return (
    ...
    >
      ...
      {/*
       * Stack vertical: avatar (en flujo) → chat (en flujo).
       * Sin gap entre ambos → base avatar = borde sup. chat.
       * El avatar wrapper tiene overflow:hidden → clip en cintura.
       */}
      <div className="relative z-10 w-full max-w-[620px] flex flex-col items-center">

        {/* ── AVATAR WRAPPER ───────────────────────────────────────────────────
         *  En flujo normal, centrado.
         *  overflow: hidden recorta todo lo que exceda los 230px de altura.
         *  La img de 320px se muestra desde arriba → solo se ve head→waist.
         *  GSAP float anima este wrapper (translateY), no el img,
         *  para que el clip se mueva completo sin cortar la cabeza.
         */}
        <div
        ...
        {/* ── CHAT BOX ─────────────────────────────────────────────────────────
         *  Sin margin-top → toca directo la base del avatar.
         *  z-10 (el avatar wrapper no tiene z-index explícito; queda delante
         *  por orden en el DOM y el transform del float).
         */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.2 }}
          className="w-full bg-white/[0.03] backdrop-blur-sm border border-white/[0.08] rounded-b-2xl rounded-t-none overflow-hidden shadow-[0_8px_40px_rgba(59,158,255,0.07)]"
        >
          {/* Header */}
          <div className="flex flex-col px-5 py-3 border-b border-white/[0.07] bg-white/[0.02]">
            ...
            <span className="text-[11px] text-slate-500 font-mono mt-0.5">
              Respondido por Isidro · IA
            </span>
          </div>

          {/* Mensajes */}
          <div className="px-4 py-4 flex flex-col gap-3 max-h-52 overflow-y-auto">
